...
class _ChangePasswordState extends State<ChangePassword> {
              ...
              child: SingleChildScrollView(
                child: Column(
                  children: <Widget>[
                    ...
                    CommonButton(
                      padding: const EdgeInsets.only(left: 24, right: 24, bottom: 16),
                      buttonText: "Apply Changes",//AppLocalizations(context).of("Apply_text"),
                      onTap: () {
                        if (_allValidation()) {
                          Navigator.pop(context);
                        }
  ...
  bool _allValidation() {
    ...
    if (_newController.text.trim().isEmpty) {
      _errorNewPassword = "Password cannot be empty";//AppLocalizations(context).of('password_cannot_empty');
      isValid = false;
    } else if (_newController.text.trim().length < 6) {
      _errorNewPassword = "Enter a valid password of more than 6 characters..";//AppLocalizations(context).of('valid_new_password');
      isValid = false;
    } else {
      _errorNewPassword = '';
    }
